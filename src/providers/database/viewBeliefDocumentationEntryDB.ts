...
@Injectable()
export class ViewBeliefDocumentationEntryDatabaseProvider {

  // views/beliefs documentation entry key
  viewBeliefDocumentationEntryCollection_key: string = 'viewBeliefDocumentationEntryCollection';

  constructor(public storage: Storage) {
    console.log('Hello ViewBeliefDocumentationEntryDatabaseProvider Provider');
  }
  ...
  getDocumentationEntryById(id: number): Promise<DocumentationEntry> {
      ...
      return this.storage.get(this.viewBeliefDocumentationEntryCollection_key).then((valArr) => {
  ...
  saveDocumentationEntry(dEntry: DocumentationEntry){
   ...
   this.storage.get(this.viewBeliefDocumentationEntryCollection_key).then(dEntryColl => {
    ...
    if(dEntryColl == null)
    {
      ...
      this.storage.set(this.viewBeliefDocumentationEntryCollection_key, viewBeliefDocumentationEntryCollection);
      ...
      if(duplicatedEntry != null)
      {
        ...
        this.deleteDocumentationEntry(duplicatedEntry.entryId).then(val => {
          if(val){
            this.storage.get(this.viewBeliefDocumentationEntryCollection_key).then(dEntryCollWADuplicate => {
              let documentationEntryCollection: DocumentationEntry[] = dEntryCollWADuplicate;
              documentationEntryCollection.push(dEntry);
              this.storage.set(this.viewBeliefDocumentationEntryCollection_key, documentationEntryCollection);
              return true;
            });
          }
        });
  ...
  deleteDocumentationEntry(id: number): Promise<boolean> {
      ...
      return this.storage.get(this.viewBeliefDocumentationEntryCollection_key).then((valArr) => {
        ...
        this.storage.set(this.viewBeliefDocumentationEntryCollection_key, newArr);
        return true;
      });
